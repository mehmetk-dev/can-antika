    ...
    return (
        <Card className="shadow-xs border-border/50">
            <CardHeader className="py-3 border-b border-border/50 flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-semibold">SipariÅŸ AkÄ±ÅŸÄ±</CardTitle>
                ...
                {recentOrders.length > 0 ? (
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <Badge className="bg-blue-500 hover:bg-blue-600 rounded">SipariÅŸ No: {recentOrders[0].id}</Badge>
                            <span className="text-xs text-muted-foreground">- Ã–deme OnayÄ± Bekliyor</span>
                        </div>
                        <div className="text-sm font-bold text-foreground">
                            {recentOrders[0].user?.name || "Misafir MÃ¼ÅŸteri"} - â‚º{recentOrders[0].totalAmount.toLocaleString("tr-TR")}
                        </div>
                        <div className="flex items-center gap-3 p-3 border rounded-md">
                            <div className="h-10 w-10 bg-muted/50 rounded flex items-center justify-center overflow-hidden">
                                {recentOrders[0].orderItems?.[0]?.product?.imageUrls?.[0] ? (
                                    <Image src={recentOrders[0].orderItems[0].product.imageUrls[0]} alt="Ã¼rÃ¼n resmi" width={40} height={40} className="h-full w-full object-cover" unoptimized />
                                ) : (
