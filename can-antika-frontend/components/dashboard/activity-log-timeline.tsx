        ...
        <Card className="shadow-sm border-stone-200 overflow-hidden bg-white">
            <CardHeader className="py-3 px-5 border-b border-stone-200 bg-stone-50">
                <div className="flex items-center gap-2.5">
                    <div className="h-7 w-7 rounded-md bg-stone-200 flex items-center justify-center">
                        <Activity className="h-3.5 w-3.5 text-stone-600" />
                    </div>
                    <CardTitle className="text-sm font-semibold tracking-tight text-stone-800">Aktivite Akışı</CardTitle>
                </div>
            </CardHeader>
                            ...
                            {activityLogs.map((log, i) => {
                                ...
                                return (
                                    <div key={i} className="relative pl-7 group">
                                        <div className={`absolute left-0 top-1.5 h-[15px] w-[15px] rounded-full border-[3px] border-white ${dotColor} ring-2 ring-stone-100`} />
                                        <div className="flex items-start justify-between gap-3">
                                            ...
                                            {timeStr && (
                                                <span className="text-[10px] text-stone-400 whitespace-nowrap tabular-nums shrink-0 mt-0.5 font-medium">
                                                    {timeStr}
                                                </span>
                                            )}
                                        </div>
