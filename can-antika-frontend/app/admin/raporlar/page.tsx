...
export default function ReportsPage() {
    ...
    const loadData = async (t: Tab) => {
        ...
        try {
            switch (t) {
                case "sales": setData(await reportApi.salesByCategory()); break
                case "stock": setData(await reportApi.stockReport()); break
                case "customers": setData(await reportApi.customerReport()); break
                case "revenue": setData(await reportApi.revenueReport(12)); break
            }
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    {(data as any[]).length === 0 && <p className="text-center text-muted-foreground py-8">Veri yok</p>}
                                    {(data as any[]).map((cat: any) => {
                                        const max = Math.max(...(data as any[]).map((c: any) => c.totalRevenue || 0), 1)
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-3">
                                    {(data as any[]).length === 0 && <p className="text-center text-muted-foreground py-8">Tüm ürünler yeterli stokta</p>}
