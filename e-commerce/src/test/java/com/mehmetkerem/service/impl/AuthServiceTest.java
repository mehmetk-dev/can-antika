...
class AuthServiceTest {
    ...
    void logoutByRefreshToken_WhenTokenExists_ShouldRevokeUserSessions() {
        when(refreshTokenService.deleteByToken("refresh-token")).thenReturn(true);

        boolean revoked = authService.logoutByRefreshToken("refresh-token");
    ...
    void logoutByRefreshToken_WhenTokenMissing_ShouldDoNothing() {
        when(refreshTokenService.deleteByToken("missing-token")).thenReturn(false);

        boolean revoked = authService.logoutByRefreshToken("missing-token");

        assertFalse(revoked);
        verify(refreshTokenService).deleteByToken("missing-token");
    }
}
